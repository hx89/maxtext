...
RUN_SETTINGS="maxtext/MaxText/train.py maxtext/MaxText/configs/base.yml run_name=${RUN_NAME}-single-run use_iota_embed=true scan_layers=true\
    steps=15 per_device_batch_size=${MBS} model_name=llama2-70b remat_policy=${POLICY} enable_checkpointing=false logits_dot_in_fp32=false\
    ...
    enable_goodput_recording=false monitor_goodput=false num_layers_per_pipeline_stage=$((80/(dcn_PP*VP)))\
...
NSYS_OUTPUT_FILE="/opt/workspace/nsys_logs/${RUN_NAME}/${RUN_NAME}-single-run"
