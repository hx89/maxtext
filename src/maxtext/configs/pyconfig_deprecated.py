...
def validate_shard_expert_on_fsdp(raw_keys):
  if not raw_keys["shard_exp_on_fsdp"]:
    return
  expert_parallelism = raw_keys["ici_expert_parallelism"] * raw_keys["dcn_expert_parallelism"]
  fsdp_parallelism = raw_keys["ici_fsdp_parallelism"] * raw_keys["dcn_fsdp_parallelism"]
  if raw_keys["num_experts"] % (expert_parallelism * fsdp_parallelism) != 0:
    raise ValueError(
        "shard_exp_on_fsdp requires num_experts to be divisible by expert_parallelism * fsdp_parallelism."
    )
  if using_tensor_parallelism(raw_keys):
