...
class MoEGeneral(BaseModel):
  ...
  te_ep_recv_capacity_factor: float = Field(
      1.0,
      description="Multiplier on TE EP worst-case recv_capacity_per_rank.",
  )
  ...
  def set_derived_and_validate_values(self) -> "MaxTextConfig":
    ...
    if self.num_experts > 1:
      ...
      if self.use_te_ep:
        ...
        if len(enabled_ep_backends) > 1:
          raise ValueError(
              "MoE EP backends are mutually exclusive; enable only one of "
              f"use_te_ep, use_hybrid_ep, or use_ring_of_experts. Got {enabled_ep_backends}."
          )
        ...
        if (
          ...
          raise ValueError("TE EP with te_mxfp8 requires moe_permutation_group_align_size to be a multiple of 128.")
